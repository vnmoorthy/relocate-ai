"""Pre-seed Supermemory with the user's "prior move" so the recall demo lights up live.
...
Run ONCE before the demo:
    uv run python scripts/seed_supermemory.py
"""
...
BUYER_PHONE = "+16184149537"  # the AgentPhone buyer number — also the lookup key
HOMEOWNER_PHONE_FALLBACK = settings.demo_homeowner_number  # for inbound caller lookups
...
async def main() -> None:
    ...
    documents = [
        {
            ...
            "content": (
                ...
                "USPS confirmation 4081-2293-6612-3128. Geico policy 4429-VC-1A. "
                "PG&E account 5512-4419-08. Comcast account 87291402. "
                "Preferred install window: 8am-noon weekdays. Bank: Wells Fargo. "
                "PCP: One Medical SF. Pharmacy: CVS Embarcadero. "
                "Relocate executed via Relocate marketplace at $2.40 in inference (PAVO-routed, "
                "61% local-tier share, 18 routing decisions)."
            ),
            "metadata": {
    ...
    async with httpx.AsyncClient(
        ...
        r = await client.post(
            ...
            json={"q": f"user_phone:{BUYER_PHONE} prior_move", "limit": 3},
        )
        r.raise_for_status()
        d = r.json()
