...
register(
    ...
    """Target: {destination_address}. Where you sleep from {move_date} and where you sign a year are two different decisions — make them in that order. Relocate does not tour, apply, sign, or send money; every step below is yours. Steps 2, 5 and 6 point at US-only resources — if {destination_address} is outside the US, ask a local contact or the local consumer-protection agency what the equivalent is before you rely on them.
...
PLAYBOOKS['housing_search'] = (
    ...
    """Housing is the one item you can start today with no account numbers and no credentials. Order matters: commute, then people, then a landing stay, then a lease. Relocate never tours, applies, signs, or sends money — these steps are yours. Steps 5 and 6 name US-only resources; for a destination outside the US, ask a local contact what the equivalent is.
...
7. Move SMS-based 2FA (bank, email, brokerage) to an authenticator app before port day — texts can go missing during the cutover. Where an institution offers SMS only, download and store its backup codes instead, and expect possible SMS gaps around the cutover.
...
9. Wi-Fi calling: 911 calls placed over Wi-Fi calling are routed using the address you register with the carrier. Carriers apply that address as soon as it is saved, so keep {origin_address} on file until you are actually living at {destination_address}, then change it. A 911 call over the cellular network is located by the network and handset, not by the address on file.
...
5. Move SMS-based 2FA to an authenticator app before port day — texts can go missing during the cutover. Where an institution offers SMS only, download its backup codes and expect possible SMS gaps around the cutover.
6. Check coverage at <your new address> on each carrier's own coverage map, by street address. Those maps are modeled predictions the carriers disclaim, so add a real check: ask neighbors or the building what works there, or use the new carrier's trial period or return window before you commit the port.
...
9. Wi-Fi calling: 911 calls placed over Wi-Fi calling are routed using the address you register. Carriers apply it as soon as it is saved, so keep <your current address> on file until you are actually living at <your new address>, then change it.
...
Sending from the US, before you fund anything: look the provider up on FinCEN's MSB registrant search, and ask which state money transmitter license covers a customer in your state, then confirm that license on NMLS Consumer Access. A listing confirms a filing or a license, not approval or endorsement — treat it as a floor, not a recommendation. Banks and bank subsidiaries do not appear in the MSB search, so a bank's absence there is not a red flag. Sending from anywhere else, the equivalent check is that country's own financial regulator's register.
...
Sending from the US: look the provider up on FinCEN's MSB registrant search, and ask which state money transmitter license covers you, then confirm it on NMLS Consumer Access. A listing means a filing or a license, not approval or endorsement. Banks and bank subsidiaries are not in the MSB search, so a bank's absence there is not a red flag. Sending from elsewhere, check that country's own financial regulator's register.
...
4. Order the essentials — beds, a table, seating — to land AFTER {move_date}, and get the ship-by date in writing before you pay. Whatever the listing promises on lead time, the written ship-by date is the only version you can hold them to. Made-to-order and upholstered pieces are the ones to order before you arrive.
...
2. Match the piece to the right numbers: CARTON dimensions for flat-pack; ASSEMBLED width, height, and diagonal depth for anything that ships assembled (sofas, upholstered chairs, mattresses, white-glove case goods). Check which set the listing gives you; if the number you need isn't there, ask the seller before ordering.
