...
| AgentMail paths | **Partial** | Six specialist email adapters exist; current policy permits mover, school, vet, gym, and bank submissions after declared prerequisites. PCP is blocked pending secure consent. A runtime destination allowlist (`AGENTMAIL_ALLOWED_RECIPIENTS`, empty by default) blocks every send whose recipient is not explicitly listed. | Secure regulated-data transport, authorized sandbox delivery and inbound reply handling, bounce/complaint/threading, and async completion tracking. |
